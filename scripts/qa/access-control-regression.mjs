...
if (!existsSync(tsNodeBin)) {
  console.error('Missing backend ts-node dependency; run npm install --prefix backend before access-control regression tests.');
  process.exit(1);
}
...
const result = spawnSync(tsNodeBin, [
  ...
  join(root, 'backend', 'test', 'access-control-regression.ts'),
], {
  cwd: root,
  stdio: 'inherit',
});
