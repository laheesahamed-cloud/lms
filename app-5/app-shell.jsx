...
const TOKENS = {
  ...
  accent:    '#3D5AFE',
  accentSoft:'#E5E9FF',
  // ─── GAME PALETTE (added for play-feel) ──────────
  gold:      '#FFB800',
  goldSoft:  '#FFF1CB',
  pink:      '#FF4D8F',
  pinkSoft:  '#FFE0EC',
  ...
  gem:       '#00BFFF',
  gemSoft:   '#D6F1FF',
  gradGold:  'linear-gradient(135deg, #FFB800 0%, #FF8A3D 100%)',
  gradPink:  'linear-gradient(135deg, #FF4D8F 0%, #FF1F5C 100%)',
  gradMint:  'linear-gradient(135deg, #00D4B4 0%, #00BFFF 100%)',
...
const Icon = ({ name, size = 22, stroke = 1.7, color = 'currentColor', style }) => {
  const paths = {
    ...
    gem:     <><path d="M5 9h14L12 21 5 9ZM5 9l3-5h8l3 5M9 9l3-5 3 5"/></>,
    crown:   <><path d="M3 8l4 3 5-6 5 6 4-3v9H3V8Z"/><circle cx="3" cy="8" r="1.2" fill="currentColor"/><circle cx="21" cy="8" r="1.2" fill="currentColor"/><circle cx="12" cy="5" r="1.2" fill="currentColor"/></>,
    bolt:    <><path d="M13 2L5 14h6l-1 8 8-12h-6l1-8Z" fill="currentColor" stroke="none"/></>,
    shield:  <><path d="M12 3l8 3v6c0 5-4 8-8 9-4-1-8-4-8-9V6l8-3Z"/></>,
    medal:   <><circle cx="12" cy="14" r="6"/><path d="M9 8 6 2h12l-3 6M12 11v3M12 17v0"/></>,
  };
  return (
...
// ─── PHONE FRAME (kept for legacy; no longer used at top-level) ───────────────
const PhoneScreen = ({ children, bg = TOKENS.bg, statusDark = false, scrollKey }) => {
  return (
    <div style={{
      ...
      <div key={scrollKey} style={{ position: 'absolute', inset: 0, overflow: 'auto', scrollbarWidth: 'none' }} className="phone-scroll">
        {children}
      </div>
    </div>
  );
...
// Mobile (<900): full-bleed, bottom tab bar fixed.
// Desktop (>=900): left sidebar (260px) + content column (max 720).
    ...
    <aside className="app-sidebar">
      <div className="sb-brand">
        <div className="sb-brand-mark" aria-hidden="true">
          <Icon name="stetho" size={20} color="#fff" stroke={2} />
        </div>
        <div>
          <div className="sb-brand-name">Caduceus</div>
          <div className="sb-brand-sub">Medical study</div>
        </div>
      </div>
      <nav className="sb-nav">
        {TABS.map(t => (
          <button key={t.id} className={`sb-item${t.id === active ? ' active' : ''}`} onClick={() => onChange(t.id)}>
            <Icon name={t.icon} size={20} stroke={t.id === active ? 2 : 1.7} />
            <span>{t.label}</span>
            {t.id === active && <span className="sb-pill" />}
          </button>
        ))}
      </nav>
      <div className="sb-footer">
        <a href="audit.html" className="sb-link">Redesign audit →</a>
        <div className="sb-user">
          <div className="sb-avatar">LA</div>
          <div style={{ minWidth: 0 }}>
            <div className="sb-user-name">Laheez</div>
            <div className="sb-user-sub">USMLE Step 1 · 47d</div>
          </div>
        </div>
      </div>
    </aside>
...
const AppHeader = ({ title, eyebrow, dark, onToggleDark, onMenu, density = 'cozy', hideMenu, hideAvatar = false }) => {
  const isDesktop = window.useMediaQuery && window.useMediaQuery('(min-width: 900px)');
  const _hideMenu = hideMenu != null ? hideMenu : isDesktop;
...
Object.assign(window, { TOKENS, Icon, PhoneScreen, AppHeader, BottomNav, Sidebar, useMediaQuery, TABS });
