...
// Apple button is hidden until a Services ID is pasted into VITE_APPLE_SERVICES_ID.
const APPLE_CONFIGURED = appleSignInConfigured();
...
export function LoginPage() {
  ...
  // Which button is mid-action ('form' | 'google' | 'apple') so ONLY that button
  // shows the spinner — not all three.
  ...
  // True while finishing an Apple redirect sign-in: the backend set the session
  // cookie and sent us back with ?apple=success. We show the same "Signing you
  // in" screen as Google, hydrate the user from the cookie, then go to dashboard.
  const [appleReturnPending, setAppleReturnPending] = useState(() => {
    if (typeof window === 'undefined' || PLATFORM.isNative) return false;
    return new URLSearchParams(window.location.search).get('apple') === 'success';
  });
  ...
  // Apple redirect return (?apple=success): the backend already set the session
  // cookie. Strip the marker, hydrate the user from it, then land on the dashboard
  // — same calm "Signing you in" screen as Google.
  const appleReturnHandledRef = useRef(false);
  useEffect(() => {
    if (!appleReturnPending || appleReturnHandledRef.current) return;
    ...
    (async () => {
      ...
      if (useAuthStore.getState().isAuthenticated) {
        ...
        setStatus({ loading: false, error: 'Apple sign-in could not be completed. Please try again.', success: '' });
      }
    })();
  }, [appleReturnPending, hydrate, navigate]);

  // Apple sign-in came back with a failure reason — surface a friendly message.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const p = new URLSearchParams(window.location.search);
    if (p.get('apple') === 'failed') {
      try { window.history.replaceState({}, '', window.location.pathname); } catch { /* ignore */ }
      setStatus({ loading: false, error: 'Apple sign-in could not be completed. Please try again.', success: '' });
    }
  }, []);
  ...
  // Finishing a Google or Apple redirect sign-in: show a calm "Signing you in"
  // screen (not the login form) until we land on the dashboard.
  if (googleReturnPending || appleReturnPending) {
    return (
      <main
        ...
        <div className="lms-login-form-wrap" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 24px 24px', overflowY: 'visible' }}>
          ...
          >
            ...
            {APPLE_CONFIGURED ? (
              <button
                type="button"
                className="lms-google-btn lms-apple-btn"
                disabled={status.loading}
                aria-busy={status.loading}
                onClick={handleAppleButtonClick}
              >
                {status.loading && pending === 'apple' ? (
                  <span className="lms-ios-loader" aria-hidden="true">
                    {Array.from({ length: 12 }, (_, index) => (
                      <span key={index} />
                    ))}
                ...
                <span>{status.loading && pending === 'apple' ? 'Signing in...' : 'Sign in with Apple'}</span>
              </button>
            ) : null}
            ...
            >
              ...
              <span>{status.loading && pending === 'google' ? 'Signing in...' : 'Continue with Google'}</span>
            </button>
