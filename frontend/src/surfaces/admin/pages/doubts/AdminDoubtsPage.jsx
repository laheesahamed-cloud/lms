...
export function AdminDoubtsPage() {
  ...
  return <main className={ui.screenShell}><section className={ui.managementLayout}>
    <AppHeader title="Lesson Doubts" subtitle="Student Questions" />
    <section className="grid gap-3">{items.length ? items.map((item) => <article className={ui.panelCard} key={item.id}><div className={ui.panelTop}><div><h2>{item.subject}</h2><p>{getAdminUserIdentifier(item)}{getAdminUserSecondaryIdentifier(item) ? ` · ${getAdminUserSecondaryIdentifier(item)}` : ''}</p><div className={doubtUi.meta}>{item.lessonTitle ? <span className={doubtUi.tag}>Lesson: {item.lessonTitle}</span> : null}{item.questionId ? <span className={doubtUi.tag}>Question #{item.questionId}</span> : null}<span className={doubtUi.tag}>{item.contextType}</span></div></div><span className={statusPill(item.status)}>{item.status}</span></div><div className={doubtUi.thread}><div className={doubtUi.bubble}><strong className="block text-ink-strong">Student</strong>{item.message}{item.questionText ? <span className="mt-2 block text-xs text-ink-muted">{item.questionText}</span> : null}</div>{item.reply ? <div className={cx(doubtUi.bubble, 'border-brand-success/20 bg-brand-success/10')}><strong className="block text-ink-strong">Admin reply</strong>{item.reply}</div> : null}</div><textarea className={ui.textarea} value={replyById[item.id] || ''} onChange={(e) => setReplyById((r) => ({ ...r, [item.id]: e.target.value }))} placeholder="Reply to student" /><div className={ui.buttonRow}><button className={ui.primaryAction} onClick={async () => { await answerLessonDoubt(item.id, { reply: replyById[item.id] || item.reply || '', status: 'answered' }); await load(); }}>Answer</button><button className={ui.secondaryAction} onClick={async () => { await answerLessonDoubt(item.id, { reply: item.reply || '', status: 'closed' }); await load(); }}>Close</button></div></article>) : <div className={ui.emptyBox}>No doubts yet.</div>}</section>
  </section></main>;
}
