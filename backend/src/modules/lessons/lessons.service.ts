...
export interface NoteSection {
  heading: string; bullets: string[]; callout: string; sticky_note: string; mnemonic: string;
  type?: string; headers?: string[]; rows?: string[][]; span?: string; steps?: string[];
  // A flow/table that belongs to THIS SAME text section's topic but can't be
  // expressed as bullets (e.g. a drug's mechanism inside the "Management"
  // section) — rendered inside the same card, below a divider, instead of
  // becoming a separate numbered card for what is really the same topic.
  embedded_flow?: string[];
  embedded_table?: { headers: string[]; rows: string[][] };
  // A short specific label for the embedded block (e.g. "GnRH agonist
  // mechanism", not the bare generic word "Mechanism") — shown on the
  // divider so a reader can tell what it's about without re-reading the
  // whole thing, especially once it's sitting inside a card that covers
  // more than one drug/topic.
  embedded_label?: string;
  // type 'note' only: an unnumbered floating box for content that doesn't
  // cleanly fit any existing topic. anchor_topic names the existing card
  // heading it should sit next to (best-effort AI judgement, not a strict key).
  anchor_topic?: string;
  // type 'branch' only: a horizontal branching diagram for classification
  // content that's fundamentally "one topic splits into named sub-types"
  // (e.g. Fibroids -> Submucosal/Intramural/Subserosal) rather than a table
...
export class LessonsService {
  ...
  // Strips any leading number an admin or a prior generation pass put on a
  // heading ("1. ", "2.3 ", "1) ") so it can be freshly renumbered.
  private stripHeadingNumber(heading: string): string {
    return String(heading || '').replace(/^\s*\d+(?:\.\d+)*[.)]?\s*/, '').trim();
  }

  private normalizeTopicKey(heading: string): string {
  ...
  // inventing its own "1.1" sub-numbering for long, structured content.
  private renumberSections(canvas: NoteCanvas): NoteCanvas {
    let n = 1;
  ...
  private groupSectionFamilies(canvas: NoteCanvas): NoteCanvas {
    ...
    for (const section of flat) {
      ...
      if (target && isText && targetIsText) {
        ...
        // A flow/table the AI embedded directly on this same section (rather
        // than as a standalone section) rides along into the merged card too.
        // Only the FIRST one wins — concatenating a second, different embedded
        // flow onto an existing one would splice two unrelated cause→effect
        // chains into a single nonsensical arrow sequence (the exact bug a
        // duplicate/overlapping AI addition produced). A duplicate addition is
        // dropped here rather than shown broken.
        if (section.embedded_flow?.length && !target.embedded_flow) {
          target.embedded_flow = section.embedded_flow;
          if (section.embedded_label) target.embedded_label = section.embedded_label;
        }
        if (section.embedded_table?.headers?.length && !target.embedded_table) {
          target.embedded_table = section.embedded_table;
          if (!target.embedded_flow && section.embedded_label) target.embedded_label = section.embedded_label;
        }
    ...
    // they followed, since an image illustrates the card above it. "note"
    // boxes are unnumbered and get pinned the same way, next to whichever
    // real card they're most related to (see anchor resolution below).
    const isImage = (s: NoteSection) => s.type === 'image' || s.type === 'image-explained';
    const isNote = (s: NoteSection) => s.type === 'note';
    ...
    // Resolve each note's anchor against the FULL set of real cards (not just
    // ones seen before it in the source), since it may come from a later
    // completeness pass but relate to an early topic. Preference order: an
    // exact heading match for anchor_topic, then a family match on
    // anchor_topic, then a family match on the note's own heading, then
    // simply wherever it already sits.
    const notesAfter = new Map<number, NoteSection[]>();
    for (const note of noteSections) {
      ...
      if (!anchor && anchorTopicNorm) {
        ...
        if (anchorFamily) anchor = reorderable.find((s) => this.topicFamily(this.stripHeadingNumber(s.heading || ''))?.key === anchorFamily.key);
      }
      if (!anchor && noteHeading) {
        const ownFamily = this.topicFamily(noteHeading);
        if (ownFamily) anchor = reorderable.find((s) => this.topicFamily(this.stripHeadingNumber(s.heading || ''))?.key === ownFamily.key);
      }
    ...
    const grouped: NoteSection[] = [...(imagesAfter.get(-1) || []), ...(notesAfter.get(-1) || [])];
  ...
  // completeness pass (missing-content audit) is its own AI call that numbers
  // its own headings "1., 2., 3." from scratch and knows nothing of the others
  // — naively concatenating produced duplicate/decimal-looking numbering and
  // scattered content (e.g. a second "Investigations" section landing at the
  // very end instead of next to the first one). So: fold a new text section
  // into an EXISTING section on the same topic instead of duplicating it —
  // final renumbering happens once, at the end of canvasGenerate.
  private mergeCanvases(canvases: NoteCanvas[]): NoteCanvas {
    const mergedSections: NoteSection[] = [];
    const sectionIndexByTopic = new Map<string, number>();
      ...
      for (const page of c.pages) {
        ...
        for (const section of page.sections) {
          ...
          if (existingIndex !== undefined) {
            ...
            if (!target.callout && section.callout) target.callout = section.callout;
            if (!target.sticky_note && section.sticky_note) target.sticky_note = section.sticky_note;
            if (!target.mnemonic && section.mnemonic) target.mnemonic = section.mnemonic;
            // First embedded flow/table wins — never splice two different ones
            // into one nonsensical chain (see groupSectionFamilies for why).
            if (section.embedded_flow?.length && !target.embedded_flow) {
              target.embedded_flow = section.embedded_flow;
              if (section.embedded_label) target.embedded_label = section.embedded_label;
            }
          ...
          const clone: NoteSection = { ...section, heading: this.stripHeadingNumber(section.heading || '') };
          mergedSections.push(clone);
          if (topicKey) sectionIndexByTopic.set(topicKey, mergedSections.length - 1);
        }
      }
    }
  ...
  private buildCompletenessPrompt(sourceText: string, coveredText: string): string {
    return [
      ...
      '- A DRUG\'s mechanism of action is ALWAYS embedded this way on the topic that uses it (almost always "Management") — NEVER return it as its own standalone {"type":"flow","heading":"..."} section, no matter how many steps it has.',
      '',
      ...
      '- EXCEPTION: if the missing content is simpler — one main topic splitting into a few named sub-types, each with just a short description and no other attribute — use {"type":"branch","root":"Main topic","branches":[{"label":"Sub-type","detail":"short description"}]} instead of a table.',
      '',
  ...
  private validate(d: unknown): NoteResult {
    ...
    return {
      ...
      sections: (Array.isArray(data?.sections) ? data.sections : []).slice(0, 40).map((s: unknown) => {
        ...
        if (String(sec?.type || '') === 'branch') {
          const branches = (Array.isArray(sec?.branches) ? sec.branches : []).slice(0, 8).map((b: unknown) => {
            const branch = (b ?? {}) as Record<string, unknown>;
            return { label: String(branch?.label || '').trim().slice(0, 60), detail: String(branch?.detail || '').trim().slice(0, 200) };
          }).filter((b) => b.label);
          return {
            type: 'branch',
            heading: String(sec?.heading || '').trim().slice(0, 160),
            root: String(sec?.root || '').trim().slice(0, 80),
            branches,
            span: String(sec?.span || 'full'),
            bullets: [], callout: '', sticky_note: '', mnemonic: '',
          };
        }
        ...
        const embedded_table = embeddedTableRaw && Array.isArray(embeddedTableRaw.headers)
          ? {
              ...
              rows: (Array.isArray(embeddedTableRaw.rows) ? embeddedTableRaw.rows : []).slice(0, 60).map((r: unknown) =>
                (Array.isArray(r) ? r : []).map(String).slice(0, 12)
              ),
            }
          : undefined;
      ...
      }).filter(s => s.heading || s.bullets.length > 0 || (s.type === 'table' && (s.headers?.length ?? 0) > 0) || (s.type === 'flow' && (s.steps?.length ?? 0) > 0) || (s.type === 'branch' && (s.branches?.length ?? 0) > 0)),
