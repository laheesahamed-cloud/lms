...
export class SchemaSyncService implements OnModuleInit {
  ...
  private async ensureEcgTables(connection: PoolConnection) {
    await connection.execute(`
  ...
  // Per-user FSRS scheduling state for flashcards. ONE row per (user_id, card_id):
  // current memory state for fast queue/due queries, plus an append-only review
  // history in log_json (for undo + later FSRS weight re-optimization). Card
  // CONTENT stays in lesson_flashcards — this table is purely per-user progress.
  private async ensureLessonFlashcardReviewsTable(connection: PoolConnection) {
    await connection.execute(`
