...
type SmartNoteRow = RowDataPacket & {
  id: number;
  user_id: number;
  title: string;
  raw_text: string | null;
  processed_qa: string | null;
  infographic_elements: string | null;
  representative_image_data: string | null;
  representative_image_prompt: string | null;
  created_at: string;
  updated_at: string;
};
...
export class SmartNotesService {
  ...
  private async getUser(token: string): Promise<UserRow> {
  ...
  async create(dto: CreateSmartNoteDto, token: string) {
    const user = await this.getUser(token);
    const [result] = await this.db.execute<ResultSetHeader>(
      'INSERT INTO smart_notes (user_id, title, raw_text) VALUES (?, ?, ?)',
      [user.id, dto.title, dto.rawText ?? null],
    );
    return { id: result.insertId, title: dto.title };
  }

  async update(id: number, dto: UpdateSmartNoteDto, token: string) {
    const user = await this.getUser(token);
    const [existing] = await this.db.execute<SmartNoteRow[]>(
      'SELECT id FROM smart_notes WHERE id = ? AND user_id = ?',
      [id, user.id],
    );
    if (!existing.length) throw new NotFoundException('Note not found');

    const fields: string[] = [];
    const values: (string | number | null)[] = [];

    if (dto.title !== undefined)                  { fields.push('title = ?');                    values.push(dto.title); }
    if (dto.rawText !== undefined)                { fields.push('raw_text = ?');                 values.push(dto.rawText); }
    if (dto.processedQa !== undefined)            { fields.push('processed_qa = ?');             values.push(JSON.stringify(dto.processedQa)); }
    if (dto.infographicElements !== undefined)    { fields.push('infographic_elements = ?');     values.push(JSON.stringify(dto.infographicElements)); }
    if (dto.representativeImageData !== undefined){ fields.push('representative_image_data = ?'); values.push(dto.representativeImageData); }
    if (dto.representativeImagePrompt !== undefined){ fields.push('representative_image_prompt = ?'); values.push(dto.representativeImagePrompt); }
  ...
  async processWithAi(id: number, token: string) {
    ...
    // Step 2: Assign positions (frontend-safe stacking — no coordinate bugs)
    ...
    // Store (cap image at 700 KB)
    const IMAGE_CAP = 700_000;
    const imageForDb = imageData && imageData.length <= IMAGE_CAP ? imageData : null;
    await this.db.execute(
      `UPDATE smart_notes SET title = ?, infographic_elements = ?, representative_image_data = ?, representative_image_prompt = ? WHERE id = ? AND user_id = ?`,
      [title, JSON.stringify(elements), imageForDb, imagePrompt, id, user.id],
    );

    // Return same cap as DB so we don't push multi-MB base64 payloads through the network
  ...
  // Assign x/y/w/h to elements using a simple vertical stacking algorithm
  private stackElements(
    raw: Array<{ id: string; type: string; text: string; color: string }>,
    title: string,
  ): CanvasElement[] {
    const CANVAS_W = 600;
    const LEFT_X = 12;
    ...
    // Title — always full width at top
    result.push({
      id: 'title',
      type: 'title',
    ...
    // Stack text elements in the left column below the title
  ...
  private normalizeType(t: string): string {
    ...
    if (s === 'section' || s === 'heading') return 'section';
    if (s === 'bullet-list' || s === 'bullet_list' || s === 'bullets') return 'bullet-list';
    if (s === 'box' || s === 'mnemonic' || s === 'comparison_box' || s === 'note_box') return 'box';
    if (s === 'callout' || s === 'tip' || s === 'note') return 'callout';
    return 'bullet-list';
  }

  private computeHeight(type: string, text: string): number {
    if (type === 'section') return 44;
    if (type === 'callout') return 72;
  ...
  private fallbackElements(rawText: string, title: string): Array<{ id: string; type: string; text: string; color: string }> {
  ...
  private fallbackSvg(seedText: string): string {
  ...
  private deserialize(row: SmartNoteRow) {
    let elements: unknown[] = [];
    try { elements = row.infographic_elements ? JSON.parse(row.infographic_elements) : []; } catch { elements = []; }
    return {
      id: row.id,
      title: row.title,
